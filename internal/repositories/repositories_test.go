...
func TestRepositories(t *testing.T) {
	...
	t.Run("scenario : create -> read -> update -> raed -> delete -> read for detailed record", func(t *testing.T) {
		...
		_, err := ReadRecord[models.Detailed](repo.AccountingDB, detailed.Model.ID)
		...
		err = UpdateRecord(repo.AccountingDB, detailed, detailed.Model.ID)

		assert.NoError(t, err, "expected no error while updating detailed record")

		fetchDetailed, err := ReadRecord[models.Detailed](repo.AccountingDB, detailed.Model.ID)
		assert.NoError(t, err, "expected no error while reading detailed record ")
	...
	t.Run("scenario : create -> read -> update -> raed -> delete -> read for subsidiary record", func(t *testing.T) {
		...
		_, err := ReadRecord[models.Subsidiary](repo.AccountingDB, subsidiary.Model.ID)
		...
		err = UpdateRecord(repo.AccountingDB, subsidiary, subsidiary.Model.ID)

		assert.NoError(t, err, "expected no error while updating subsidiary record")

		fetchSubsidiary, err := ReadRecord[models.Subsidiary](repo.AccountingDB, subsidiary.Model.ID)
		assert.NoError(t, err, "expected no error while reading subsidiary record ")
	...
	t.Run("scenario : create -> read -> update -> raed -> delete -> read for voucher record", func(t *testing.T) {
		...
		_, err := ReadRecord[models.Voucher](repo.AccountingDB, voucher.Model.ID)

		assert.NoError(t, err, "expected no error while reading voucher record ")

		fetchVoucherItem1, err := ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem1.Model.ID)
		...
		fetchVoucherItem2, err := ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem2.Model.ID)
		...
		err = UpdateRecord(repo.AccountingDB, voucher, voucher.Model.ID)

		assert.NoError(t, err, "expected no error while updating voucher record")

		fetchVoucher, err := ReadRecord[models.Voucher](repo.AccountingDB, voucher.Model.ID)
		assert.NoError(t, err, "expected no error while reading voucher record ")
		...
		_, err = ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem1.Model.ID)
		assert.Error(t, err, "expected  error indicate voucherItem1 record not found  ")

		_, err = ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem2.Model.ID)
	...
	t.Run("scenario : create -> read -> update -> raed -> delete -> read for voucher item record", func(t *testing.T) {
		...
		err = UpdateRecord(repo.AccountingDB, fetchVoucherItem1, fetchVoucherItem1.Model.ID)

		assert.NoError(t, err, "expected no error while updating voucherItem1 record")

		err = UpdateRecord(repo.AccountingDB, fetchVoucherItem2, fetchVoucherItem2.Model.ID)

		assert.NoError(t, err, "expected no error while updating voucherItem2 record")

		updatedVoucherItem1, err := ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem1.Model.ID)
		assert.NoError(t, err, "expected no error while reading voucherItem1 record ")
		assert.Equal(t, updatedVoucherItem1.Debit, fetchVoucherItem1.Debit)

		updatedVoucherItem2, err := ReadRecord[models.VoucherItem](repo.AccountingDB, voucherItem2.Model.ID)
		assert.NoError(t, err, "expected no error while reading voucherItem2 record ")
		...
		_, err = ReadRecord[models.VoucherItem](repo.AccountingDB, updatedVoucherItem1.Model.ID)
		assert.Error(t, err, "expected  error indicate voucherItem1 record not found  ")
		...
		_, err = ReadRecord[models.VoucherItem](repo.AccountingDB, updatedVoucherItem2.Model.ID)
		assert.Error(t, err, "expected  error indicate voucherItem2 record not found  ")
